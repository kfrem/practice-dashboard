...
<div id="app">
  ...
  <nav>
    ...
    <div class="nav-right">
      ...
      <button class="btn btn-outline" style="font-size:13px;padding:8px 16px" onclick="openM('onboardModal')">ðŸ“‹ Send Onboarding Link</button>
  ...
  <div class="main">
    ...
    <div class="overdue-bar" id="overdueBar">
      <div>
        <div class="overdue-bar-text">âš ï¸ <span id="overdueCount">0</span> client(s) have not signed past their deadline</div>
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap">
        ...
        <button class="btn btn-outline" style="color:#991b1b;border-color:#fca5a5" onclick="switchTab('overdue', document.getElementById('tabOverdue'))">View Overdue â†’</button>
      </div>
    </div>
      ...
      <div class="table-wrap">
        <div class="table-top">
          ...
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            <input type="text" id="searchBox" placeholder="Searchâ€¦" style="padding:7px 12px;border:1px solid var(--border);border-radius:4px;font-size:13px;outline:none;width:180px" oninput="renderTable()">
    ...
    <div id="viewMtd" style="display:none">
      <div class="table-wrap">
        <div class="table-top">
          ...
          <div style="font-size:12px;color:var(--muted)">MTD for Income Tax mandatory from April 2026 for income over Â£50,000</div>
        </div>
        <div style="padding:16px 20px;background:#fffbeb;border-bottom:1px solid var(--border)">
          <div style="font-size:13px;color:#92400e;font-weight:600;margin-bottom:4px">âš ï¸ MTD for Income Tax is now live â€” 6 April 2026</div>
          <div style="font-size:12px;color:#b45309">Clients earning over Â£50,000 from self-employment or property must now keep digital records and submit quarterly updates. Threshold drops to Â£30,000 in April 2027.</div>
        </div>
        <table>
          <thead>
      ...
      <div class="table-wrap">
        <div class="table-top">
          ...
          <div class="dl-filters">
            ...
            <button class="dl-filter" onclick="dlFilter('overdue',this)">ðŸ”´ Overdue <span id="dlCountOverdue" class="dl-count" style="display:none"></span></button>
        ...
        <div style="padding:12px 20px;background:#eff6ff;border-bottom:1px solid #bfdbfe;font-size:12px;color:#1e40af">
          â„¹ï¸ Set up deadline tracking per client using the <strong>ðŸ“…</strong> button on any client row. Deadlines auto-calculate from year end dates.
        </div>
        <table>
          <thead>
  ...
  <footer>
    <strong>The Practice</strong> Â· practice.finaccord.pro Â· info@kafs-ltd.com Â· GDPR-compliant Â· UK-hosted Â· Strictly confidential &nbsp;|&nbsp; Â© 2026 A Finaccord Professional Services Programme
  </footer>
</div>
...
<div class="overlay" id="addModal">
  <div class="modal">
    <div class="m-head"><div class="m-title">Add New Client</div><button class="m-close" onclick="closeM('addModal')">Ã—</button></div>
    <div class="m-body">
      <div id="addAlert"></div>
      <div class="fg-row">
      ...
      <div class="fg">
        <label>Company / Trading Name
          <span style="font-weight:400;text-transform:none;letter-spacing:0;font-size:11px;color:var(--muted)"> â€” type name then click Lookup to auto-fill from Companies House</span>
        </label>
        <div style="display:flex;gap:8px">
          <input type="text" id="addCompany" placeholder="Smith Ltd" style="flex:1">
          <button type="button" class="btn btn-outline" onclick="chLookup()" style="white-space:nowrap;padding:8px 14px;font-size:13px">ðŸ” CH Lookup</button>
        </div>
      ...
      <div class="fg">
        <label>Services <span style="font-weight:400;text-transform:none;letter-spacing:0;color:var(--muted)">(tick all that apply â€” add custom below)</span></label>
        <div class="svc-picker">
          ...
          <div class="svc-custom-row">
            <input type="text" id="addCustomSvc" placeholder="Add a custom service not listed aboveâ€¦" onkeydown="if(event.key==='Enter'){event.preventDefault();addCustomSvc()}">
...
<div class="overlay" id="linkModal">
  <div class="modal modal-wide">
    ...
    <div class="m-body">
      ...
      <!-- STEP 1: EDIT LETTER -->
      <div id="linkStep1">
        <p style="font-size:13px;color:var(--muted);margin-bottom:16px">Sending to: <strong id="linkEmail"></strong> â€” Edit the letter below before sending. The client will see exactly this.</p>
          ...
          <div class="firm-sig-section">
            ...
            <div class="fsig-panel" id="fsigPanelSaved">
              <div class="fsig-preview-box">
                <span id="fsigNoSig" style="font-size:12px;color:var(--muted)">No signature saved yet â€” use Draw or Upload to add one.</span>
            ...
            <div class="fsig-panel" id="fsigPanelDraw" style="display:none">
              ...
              <button type="button" class="btn-save-sig" onclick="saveFirmSig('draw')">ðŸ’¾ Save &amp; Use This Signature</button>
            ...
            <div class="fsig-panel" id="fsigPanelUpload" style="display:none">
              ...
              <button type="button" class="btn-save-sig" onclick="saveFirmSig('upload')">ðŸ’¾ Save &amp; Use This Signature</button>
      ...
      <div id="linkStep2" style="display:none">
        <div class="alert a-ok" style="font-size:14px;padding:14px">âœ“ Email sent successfully. The client will receive their signing link shortly.</div>
...
<div class="overlay" id="mtdModal">
  <div class="modal">
    <div class="m-head"><div class="m-title">MTD Record â€” <span id="mtdClientName"></span></div><button class="m-close" onclick="closeM('mtdModal')">Ã—</button></div>
    <div class="m-body">
      <div id="mtdAlert"></div>
      <div class="fg-row">
      ...
      <div class="fg" style="border:1px solid var(--border);border-radius:6px;padding:14px 16px;background:#fafaf8;">
        <label style="display:block;margin-bottom:10px;font-weight:700;color:var(--navy-dark)">ðŸ”” Automatic Submission Reminders</label>
        ...
        <div style="font-size:12px;color:var(--muted)">ðŸ’¡ Requires cron job to be active on the server â€” see setup guide.</div>
      </div>
      ...
      <div class="alert a-warn" style="font-size:12px">
        MTD for Income Tax requires clients earning over Â£50,000 from self-employment or property to keep digital records and submit quarterly updates to HMRC using compatible software from 6 April 2026.
      </div>
    </div>
    <div class="m-foot">
...
<div class="overlay" id="onboardModal">
  <div class="modal">
    <div class="m-head"><div class="m-title">ðŸ“‹ Send Client Onboarding Link</div><button class="m-close" onclick="closeM('onboardModal')">Ã—</button></div>
    <div class="m-body">
...
<div class="overlay" id="dlModal">
  <div class="modal">
    <div class="m-head"><div class="m-title">ðŸ“… Deadline Setup â€” <span id="dlClientName"></span></div><button class="m-close" onclick="closeM('dlModal')">Ã—</button></div>
    <div class="m-body">
      <div id="dlAlert"></div>
...
<div class="overlay" id="notesModal">
  <div class="modal">
    <div class="m-head"><div class="m-title">Internal Notes</div><button class="m-close" onclick="closeM('notesModal')">Ã—</button></div>
    <div class="m-body">
      <div id="notesAlert"></div>
      <p style="font-size:13px;color:var(--muted);margin-bottom:12px">Notes for: <strong id="notesClientName"></strong><br><span style="font-size:11px">Internal only â€” not visible to client</span></p>
      <div class="fg">
        <label>Notes</label>
...
<div class="overlay" id="amlModal">
  <div class="modal">
    <div class="m-head"><div class="m-title">AML â€” Customer Due Diligence</div><button class="m-close" onclick="closeM('amlModal')">Ã—</button></div>
    <div class="m-body">
      <div id="amlAlert"></div>
      ...
      <div class="fg"><label>CDD Notes</label>
        <textarea id="amlNotes" placeholder="Record how identity was verified, any enhanced due diligence conducted, and any concerns notedâ€¦"></textarea>
      </div>
      <div class="alert a-warn">
        âš ï¸ You remain legally responsible for client identity verification under the Money Laundering Regulations 2017. This record assists your compliance recordkeeping â€” it does not constitute verification itself.
      </div>
    </div>
    <div class="m-foot">
